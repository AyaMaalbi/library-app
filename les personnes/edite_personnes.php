...
if (isset($_GET["id"]) && $_GET["id"] != '') {
    ...
    $id = $_GET["id"];
    $sql = "select *  from lespersonnes where personne=$id ";
    $st = $bd->query($sql);
    if ($st == false) {
        header("Location: list_personnes.php?msg=err_id");
    }
    $personnes = $st->fetch();
}
...
if (isset($_POST['Enregistrer'])) {
    ...
    $sql = "UPDATE  `lespersonnes` SET  `nom`='$nom', `prenom`='$prenom', `datenaissance`='$dateN' where personne=$id";
    $st = $bd->exec($sql);
    if ($st == true) {
        header("Location: list_personnes.php?msg=modifié");
    }
}
                ...
                <div class="card-body">
                    ...
                    <form method="post">
                        <div class="mb-3">
                            ...
                            <input type="text" name="personne" value="<?= $personnes['personne'] ?>" class="form-control" disabled>

                        </div>
                        <div class="mb-3">
                            <label for="exampleInputEmail1" class="form-label">nom</label>
                            <input type="text" name="nom" value="<?= $personnes['nom'] ?>" class="form-control" >

                        </div>
                        <div class="mb-3">
                            <label for="exampleInputEmail1" class="form-label">prenom</label>
                            <input type="text" name="prenom" value="<?= $personnes['prenom'] ?>" class="form-control" >

                        </div>
                      
                        <div class="mb-3">
                            <label for="exampleInputPassword1" class="form-label">date de naissance</label>
                            <input type="date" name="dateN" value="<?= $personnes['datenaissance'] ?>" class="form-control">
                        </div>
